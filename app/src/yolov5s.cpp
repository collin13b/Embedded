...
int yolov5s::inference_img(Mat &img)
{
    ...
    try{
        ...
        Mat output_mat(model_height,model_width,CV_8UC3,dst_buf.get_ptr());
        imwrite("dst.jpg",output_mat);
        Mat output_mat1(model_height,model_width,CV_8UC3,src_buf.get_ptr());
        imwrite("src.jpg",output_mat1);
        Mat output_mat2(model_height,model_width,CV_8UC3,cvt_buf.get_ptr());
        imwrite("cvt.jpg",output_mat2);
        //设置输入
